...
import { ArrowLeft, Send, Moon, Settings, CheckCircle, Circle } from 'lucide-react';
...
export default function Reflection() {
  ...
  async function initSession() {
    ...
    try {
      ...
      if (existingMessages.length > 0) {
        ...
        // Re-mark chip messages as used (all past AI messages with chips)
  ...
  async function sendMessage(userText, overrideSessionId, overrideState) {
    ...
    setMessages((prev) => [...prev.filter((m) => !m.isTyping), { id: typingId, role: 'assistant', isTyping: true }]);
    setIsLoading(true);
    ...
    try {
      ...
      const response = await fetch('/api/reflection-chat', {
        ...
        body: JSON.stringify({
          ...
          context: {
            ...
            time_of_day: timeContext.period,
            yesterday_commitment: yesterdayCommitment,
          },
        }),
      });
      ...
      // Collect extracted data for summary card
      ...
      // Update messages — remove typing indicator, add AI message
      setMessages((prev) => {
        const without = prev.filter((m) => !m.isTyping);
        return [...without, aiMessage];
      });

      // Fire-and-forget save AI message
      reflectionHelpers
        .saveMessage(sid, user.id, {
          role: 'assistant',
          content: data.assistant_message || '',
          ...
          chips: data.chips || null,
          extracted_data: isSummaryCard ? { ...data.extracted_data, card_data: newSummaryData } : data.extracted_data,
        })
        .catch(() => {});
    ...
    <div className="flex flex-col h-screen bg-zinc-950 text-white">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-800 bg-zinc-950 flex-shrink-0">
        ...
        <div className="text-center">
          <h1 className="text-white font-semibold text-sm tracking-wide">Nightly Reflection</h1>
          <p className="text-zinc-500 text-xs">{timeContext.greeting}</p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="w-7 h-7 rounded-full bg-red-700 flex items-center justify-center text-xs font-semibold text-white">
            {userInitial}
          </div>
        </div>
      </div>
