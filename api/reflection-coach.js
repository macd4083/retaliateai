...
Set directive_completed: "${probeId}" when done.`,
  ...
  if ((currentStage === 'commitment_checkin' || isTransitioningToCheckin) && !sessionState.commitment_checkin_done && yc) {
      ...
      const checklistText = checklistFragments.length > 0 && messageCount === 0
        ? '\n- IMPORTANT: This is the very first message of the session. Do NOT mention the checklist, commitments, or ask the user to check anything off. The checklist UI will appear automatically after they respond. Just greet them warmly and ask how they are doing tonight — e.g. "Hey, how\'s it going?" or similar. Keep the opener warm and open-ended. Set commitment_checkin_done: false.'
        : (checklistFragments.length > 0
          ? (() => {
      ...
      allDirectives.push({
        ...
        instruction: messageCount === 0
          ? `## STAGE: COMMITMENT CHECK-IN (INIT)\nThis is the very first message of the session. A checklist of yesterday's commitments will appear for the user to check off after they respond.\n- Greet them warmly and ask how they're doing or how their day went. One simple, human question.\n- Do NOT mention yesterday's commitment, present any data, or reference what they committed to.\n- Do NOT use coaching-speak. Just a genuine, warm opener.\n- Do NOT set commitment_checkin_done: true yet — wait for the checklist to be submitted.`
  ...
  if (currentStage === 'tomorrow' && !sessionState.tomorrow_commitment) {
    ...
    if (rate !== null && rate < 50) {
      ...
      minimumFraming = honestMoment
        ? `They just admitted "${honestMoment.slice(0, 60)}...". This is your framing anchor. Ask for the MINIMUM floor commitment first — not everything, just what they can guarantee. Frame the question around the tension they identified in the honest stage (e.g. short-term comfort vs long-term progress, avoidance vs showing up, false control vs real effort). Example framing: "Given what you just admitted about [their pattern], what's the minimum amount of work you're committing to do tomorrow that's actually moving you toward your goals? Start with the floor — not everything you want to do, just what you know for certain you'll get done." IMPORTANT: This is asking for the MINIMUM first — do NOT ask for everything at once, do NOT say "walk me through the list", do NOT use vague language like "small step" or "what can you do". Use the specific pattern or honest insight they named as the framing — make this feel personally connected to what they just revealed.`
...
Set directive_completed: "${probeId}" when done.`,
  ...
  if (suggestedNextStage && !isMemoryMode) {
    ...
    const stageHintInstruction = (isWinsToHonest && userSignaledDone)
      ...
      : isHonestToTomorrow
        ? `STAGE HINT: Ready to move to "tomorrow". Transition naturally if conversation supports it — use a soft bridging phrase that signals the shift without announcing it (e.g. "Alright, I've got a good picture of today. Let's talk about tomorrow."). For honest→tomorrow: Only transition if honest_depth has been confirmed for this session. Do NOT transition just because the user mentioned something they want to do differently — that belongs in honest. Wait for explicit stage_advance signal. Never announce the stage name. Set stage_advance:true, new_stage:"tomorrow". CRITICAL — FIRST QUESTION IN TOMORROW STAGE: The very first question you ask after transitioning to tomorrow MUST ask for the user's MINIMUM commitment floor — not a generic "small step" or "next step". Frame it using the honest moment they just shared: connect their specific pattern or struggle (e.g. prioritizing short-term comfort over long-term progress, avoidance, false control) to the minimum floor question. Example: "Given what you just shared about [their honest pattern], what's the minimum amount of work you're committing to do tomorrow that's actually moving you toward your goals? Start with the floor — not everything you want to do, just what you know for certain you'll get done." Do NOT open tomorrow with a vague question like "What's one small step you can take?" or "What's a next step?" — those do not invite minimum-first framing and are too disconnected from the honest insight.`
